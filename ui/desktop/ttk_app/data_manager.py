...
class DataManager:
    ...
    def delete_event(self):
        ...
        if response:
            try:
                success, message = db_manager.delete_event(self.selected_event.id)
                if success:
                    messagebox.showinfo("成功", f"事件删除成功！\n\n{message}")
                    self.refresh_data()
                    self.status_label.config(text="事件删除成功")
                    self.selected_event = None
                else:
                    messagebox.showerror("错误", f"删除事件失败:\n{message}")
            except Exception as e:
                messagebox.showerror("错误", f"删除事件时发生错误:\n{e}")
