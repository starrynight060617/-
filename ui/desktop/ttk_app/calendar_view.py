...
class CalendarView:
    ...
    def load_events_data(self):
        ...
        try:
            ...
            end_date = date(current_year + 1, 12, 31)  # 包含明年
            ...
            print(f"📊 成功加载 {len(events)} 个事件，分布在 {len(self.events_data)} 个日期")
    ...
    def on_date_click(self, clicked_date):
        ...
        if self.selected_cell:
            self.selected_cell.configure(relief='raised', bd=1)
            # 这里需要根据日期状态重新设置背景色
            # 简化处理，在实际使用中可能需要重新渲染单元格
